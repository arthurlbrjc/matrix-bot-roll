...
async def handle_room_message(
    ...
    if command_name in _DETAIL_ALIASES:
        reply = _handle_detail(room.room_id)
...
def _handle_detail(room_id: str) -> str:
    """Return the full breakdown of the last roll shown in `room_id` for `!detail`/`!d`, or NO_PREVIOUS_ROLL if there isn't one."""
    stored = _last_details.get(room_id)
    if stored is None:
        return NO_PREVIOUS_ROLL
    result, message = stored
    return _format_result(result, verbose=True, message=message)


async def _handle_roll(
    client: AsyncClient, room_id: str, user_id: str, body: str
) -> str:
    """Parse and roll a `!roll`/`!r`/`!reroll`/`!rr` command, remembering the result for a later `!detail`, and return the reply text."""
    parsed = await _resolve_dice_command(client, room_id, user_id, body)
    if isinstance(parsed, str):
        return parsed
    result = handle(parsed.command)
    _last_details[room_id] = (result, parsed.message)
    return _format_result(result, verbose=parsed.verbose, message=parsed.message)
