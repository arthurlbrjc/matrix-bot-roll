...
- Before considering a task done, invoke the project's `code-review` subagent (via the Agent tool) and address its findings — `/code-review` itself can't be triggered by Claude, only by the user.